> **Doc histórica de persona.** Esta narrativa describe cómo se razonó el rol de Backend/Infra durante el MVP. **No es un subagente activo de Claude Code.** Los subagentes operativos viven en `.claude/agents/`. Mantenida como referencia de portafolio y para preservar el razonamiento original. La fuente operativa actual de reglas es `CLAUDE.md` en la raíz.
